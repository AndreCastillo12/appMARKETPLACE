        ...
        database.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (productSnapshot in snapshot.children) {
                    Log.d("ManageProductsActivity", "ProductSnapshot: ${productSnapshot.value}")
                    val product = productSnapshot.getValue(ModeloAnuncio::class.java)
                    product?.let {
                        it.id = productSnapshot.key ?: ""
                        productList.add(it)
                        Log.d("ManageProductsActivity", "Product added: $it")
                    } ?: Log.e("ManageProductsActivity", "Product is null for snapshot: ${productSnapshot.key}")
                }
                productAdapter.notifyDataSetChanged()
            }

            override fun onCancelled(error: DatabaseError) {
                Log.e("ManageProductsActivity", "DatabaseError: ${error.message}")
            }
        })
    }
    ...
    private fun eliminarProducto(product: ModeloAnuncio) {
        database.child(product.id).removeValue().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                Log.e("ManageProductsActivity", "Error al eliminar el producto: ${task.exception?.message}")
            }
        }
    }
}
